...
export function BookEditDialog({ content, open, onOpenChange, onSuccess }: BookEditDialogProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await editContent(content.id, content.status, updatePayload, newFile, newCoverImage, content.title, content.file_url, 'book');
            onSuccess();
            onOpenChange(false);
        } catch (e: any) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>{t('dashboard.myContent.edit.title')} (کتاب)</DialogTitle>
                </DialogHeader>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        <div className="space-y-2 md:col-span-3">
                            ...
                            <div className="border-2 border-dashed border-border rounded-lg px-4 text-center h-[110px] flex items-center justify-center">
                                <input id="edit-book-file" type="file" accept=".pdf,.epub,.doc,.docx" onChange={(e) => setNewFile(e.target.files?.[0] || null)} className="hidden" />
                                <label htmlFor="edit-book-file" className="cursor-pointer w-full text-sm text-muted-foreground flex flex-col items-center gap-1">
                                    ...
                                    <span className="max-w-[80%] truncate text-center font-medium">
                                        {newFile ? newFile.name : (content?.title || t('dashboard.upload.clickToUpload', { type: t('nav.books') }))}
                                    </span>
